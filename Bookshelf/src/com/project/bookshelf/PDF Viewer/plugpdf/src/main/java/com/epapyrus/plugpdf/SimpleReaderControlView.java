...
public class SimpleReaderControlView extends RelativeLayout {
	...
	private AnnotTextMarkupSettingMenu mAnnotTextMarkupSettingMenu;
	Activity mAct;
	...
	public void init(Activity act, BaseReaderControl controller) {
		
		mAct = act;
		mController = controller;
		mAnnotSettingMenu = new AnnotSettingMenu(getContext());
		mAnnotTextMarkupSettingMenu = new AnnotTextMarkupSettingMenu(getContext());

		mControlPanel = new SimpleReaderControlPanel(getContext(), this,
				mController);

		if (mController.getOutlineItem() != null) {
	...
	// set the display view mode in the bilateral direction.
	void setBilateralMode() {
		mPageDisplayModeButton
				.setBackgroundResource(R.drawable.st_btn_view_mode_bilateral);
		mController.setPageDisplayMode(PageDisplayMode.BILATERAL);
		showBottomPannel();
	}
	...
	private void showBottomPannel() {
		mPageNumberView.setVisibility(View.VISIBLE);
		mPageSlider.setVisibility(View.VISIBLE);
		mPageThumbnail.setVisibility(View.VISIBLE);
	}

	private void hideBottomPannel() {
		mPageNumberView.setVisibility(View.INVISIBLE);
		mPageSlider.setVisibility(View.INVISIBLE);
		mPageThumbnail.setVisibility(View.GONE);
	}
	...
	private class EditButtonClickHandler implements View.OnClickListener,
		...
		public boolean onLongClick(View v) {
			...
			if (v.getId() == mEditInkButton.getId()) {
				mAnnotSettingMenu.show(v, 0, 0);
			} else if (v.getId() == mEditHighlightButton.getId()) {
				mAnnotTextMarkupSettingMenu.show(v, 0, 0, AnnotToolType.HIGHLIGHT);
			} else if (v.getId() == mEditUnderlineButton.getId()) {
				mAnnotTextMarkupSettingMenu.show(v, 0, 0, AnnotToolType.UNDERLINE);
			} else if (v.getId() == mEditStrikeoutButton.getId()) {
				mAnnotTextMarkupSettingMenu.show(v, 0, 0, AnnotToolType.STRIKEOUT);
			}
			return false;
		}
